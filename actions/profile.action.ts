...
const updateProfileSchema = z.object({
	...
	phone: z.string().regex(/^[+]?[1-9][\d\s\-()]{7,15}$/, "Invalid phone number").optional().or(z.literal("")),
});
...
export async function getProfile() {
	try {
		...
		// No need to parse JSON fields for legal platform
...
export async function deleteAccount(data: DeleteAccountInput) {
	try {
		...
		await prisma.$transaction(async (tx) => {
			// Delete user's legal consultations
			await tx.legalConsultation.deleteMany({
